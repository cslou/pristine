...
export function parseRunArgs(args: string[]): RunArgs | null {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "-p" || arg === "--provider") {
      ...
      parsed.force = true
    } else if (arg === "--temporal-mode") {
      parsed.temporalMode = args[++i]
    } else if (arg === "--question-types") {
      parsed.questionTypes = args[++i].split(",")
    }
  }
...
export async function runCommand(args: string[]): Promise<void> {
  ...
  if (!parsed) {
    ...
    console.log("  --temporal-mode        Temporal query mode: current (default), as_of, full")
    console.log("  --question-types       Comma-separated question types to filter (e.g. temporal-reasoning,knowledge-update)")
