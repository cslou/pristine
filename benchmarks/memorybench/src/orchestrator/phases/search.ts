...
const TEMPORAL_MODE_MAP: Record<string, string> = {
  "knowledge-update": "current",
  "temporal-reasoning": "as_of",
}

function resolveTemporalMode(runMode: string | undefined, questionType: string): string | undefined {
  if (runMode !== "auto") return runMode
  return TEMPORAL_MODE_MAP[questionType] ?? "current"
}
  ...
  await ConcurrentExecutor.execute(
    ...
    async ({ item: question, index, total }) => {
      ...
      try {
        ...
        const resolvedMode = resolveTemporalMode(checkpoint.temporalMode, question.questionType)
        const results = await provider.search(question.question, {
          containerTag,
          limit: 10,
          threshold: 0.3,
          temporalMode: resolvedMode,
          asOf: resolvedMode === "as_of" ? questionCheckpoint?.questionDate : undefined,
        })
