...
### 1A.4 Pi session-relay reference addendum

Sprint 032 adds a Pi-first `session-relay` reference alongside the source-pointer semantic index. It is intentionally independent of the vector index: no embeddings, no `sqlite-vec`, no `jsonl-index`, no `search-memory`, and no `pristine_recall` are required. The relay owns a lightweight generic `memory_sessions` metadata contract with `source_harness`, `source_uri`, `cwd`, first/last visible-message timestamps, visible count, active-entry metadata, and `updated_at`.

This does not change the source-pointer semantic index boundary. Pristine still does not mirror raw transcripts or own harness conversations. `session-relay` stores only session-level metadata and reads bounded visible text from the authoritative Pi JSONL source when a new-session handoff is needed. The current reference writes `source_harness = 'pi'`; Codex/Claude adapters are deferred and must preserve the same source-owned boundary if added later.

### 1A.5 SQL primitive removal
