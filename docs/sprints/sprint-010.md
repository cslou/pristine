...
### Parallelization
Stories 1-2 (Ollama embedder) are sequential. Story 3 (temporalMode API) is independent and can run in parallel with Stories 1-2.
...
- **Technical notes:**
  - The `ModelConfig` type in `src/core/init.ts` gains `embedder?: EmbedderEntry` where `EmbedderEntry = { engine: 'ollama', model: string, host?: string } | { engine: 'llamacpp', path: string } | { engine: 'local', model?: string }`
  - `validateEmbedderEntry()` follows the same pattern as `validateModelEntry()`
  - `PristineLocal.create()` flow: `config.embedder ?? createEmbedder(modelConfig.embedder ?? { engine: 'local' })`
  ...
  - Update the `DEFAULT_MODEL_CONFIG` to include `embedder: { engine: 'ollama', model: 'nomic-embed-text' }` — this is the default for new installs. Existing `models.json` without an `embedder` key falls back to `{ engine: 'local' }` (backward compat).
...
- **Acceptance criteria:**
  ...
  - [ ] Default `temporalMode` is `'current'` — only return facts that are still true
...
- **Technical notes:**
  ...
  - **Breaking change:** `search(query, userId, 10)` no longer compiles — must be updated to `search(query, userId, { topK: 10 })`. Known call sites: `tests/integration/orchestrator.test.ts:151`, `tests/e2e/sdk.test.ts`. Update these in commit 1 alongside the signature change.
