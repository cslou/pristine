...
- **Current state:** Main carries the Sprint 009 `ConversationStore`, Sprint 010 embedder engines, the Sprint 011 IngestQueue, the spec-004 privacy subsystem, and the LOCOMO-aimed extraction pipeline (extractor + consolidator + episodes + graph + temporal + query-analyzer + memory-store). PR #96 just landed spec-005, which pivots memory to a corpus-based architecture. Phase 1 of spec-005 removes the LOCOMO-aimed subsystem entirely.
...
- **Planned commits:**
  1. `chore: delete src/memory/extractor and its tests` — whole-module removal
  2. `chore: delete src/memory/consolidator and its tests` — whole-module removal
  3. `refactor: remove Extractor and Consolidator interfaces from src/core/interfaces.ts`
  4. `refactor: remove fact-pipeline types from src/core/types.ts` (Fact, ExtractionResult, ConsolidationResult, ConsolidationRequest, ConsolidationBatchResult, ExtractorConfig, etc.)
  5. `refactor: remove extractor/consolidator exports from src/index.ts`
- **Technical notes:**
  - `ExtractorConfig` was exported from the public API in PR #94; callers (if any) need migration — check `npm search @pristine` usage and README examples; the spec-005 pivot deprecates this entry point entirely.
...
- **Acceptance criteria:**
  ...
  - [ ] `chunker.ts` and `ranking.ts` compile in isolation (no dead imports to removed modules)
  - [ ] `npm run typecheck` passes
...
- **QA:** N/A — backend-only removal.
- **Planned commits:**
  1. `chore: delete src/memory/store and tests (fact-ledger)` — whole-module removal
  2. `refactor: remove memory-Store interface from src/core/interfaces.ts`
  3. `chore: partial-delete src/memory/orchestrator — remove pipeline, retrieve, ingest, turn-order; preserve chunker`
  4. `chore: partial-delete src/memory/retriever — remove fact-retrieval index; preserve ranking`
  5. `refactor: update types.ts for orphaned references in preserved files` (if chunker/ranking pulled types that now need inlining or migrating)
- **Technical notes:**
...
- **Acceptance criteria:**
  - [ ] `src/client.ts` — `PristineLocal.create()` no longer wires extractor, consolidator, or memory-`Store`; dependencies reduced to conversations, embedder, queue, privacy
...
- **QA:**
  - Manual: `cd /tmp && mkdir smoke && cd smoke && npm init -y && npm install ../../pristine && node -e "import('@pristine/shield-local').then(m => { const c = m.PristineLocal.createLite({ dbPath: './test.db', userId: 'u1' }); console.log(Object.keys(c)); })"` — confirm it imports and instantiates.
  - Automated: N/A.
- **Planned commits:**
  1. `refactor: drop extractor/consolidator/memory-store wiring from PristineLocal.create()`
  2. `refactor: finalize src/index.ts — remove all references to deleted modules`
  3. `test: update client integration tests — LLM client no longer required for create()`
  4. `docs: note Phase 1 completion in README — SDK surface is now corpus + privacy + queue only`
- **Technical notes:**
  ...
  - Check `benchmarks/memorybench/src/providers/pristine/` for provider code that constructs `PristineLocal` — update the memorybench provider to the post-Phase-1 API shape, or flag the failing benchmark for sprint-017 (benchmark rebuild).
- **Priority:** Must-have
