...
- **Acceptance criteria:**
  ...
  - [ ] **Corpus: synthesized + sanitized.** `tests/integration/embedder-eval/corpus.jsonl` — one JSON object per line with `{ id, content, role, conversation_id, project_id }`. Synthesized via LLM (Claude) by paraphrasing public-domain technical-conversation content into the 3 grade-buckets (deviation from the original "real session logs" plan: maintainer's own logs are out of scope for this sprint, deferred to a follow-up "real-corpus" sprint). Passed through `secureAndRedact` before commit so any incidental PII / secrets cannot land in the repo. Document the synthesis approach + regeneration recipe in the doc header.
  - [ ] **LLM-only labels.** All query/document pairs are judged by an LLM (Claude) against a documented rubric in `tests/integration/embedder-eval/labelling-notes.md`. No human-seed / spot-check this sprint (deviation from the original plan: maintainer hand-judgment is deferred to a follow-up "real-eval" sprint after the harness is mature). The labelling-notes.md records: rubric used, model invoked, generation timestamp, and a note that Story 4's eval results carry the LLM-judge-only caveat.
...
- **Functional verification:**
  ...
  - [ ] `tests/integration/embedder-eval/labelling-notes.md` records the LLM-judge rubric, model invoked, and the deviation note (no maintainer seed / spot-check this sprint; deferred to follow-up).
- **Regression verification:**
