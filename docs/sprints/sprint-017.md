...
- **Functional verification:**
  - [ ] Default-dim regression: `npm run test:integration` with no test changes passes — every existing test that doesn't pass an explicit `dim` gets 768 and works exactly as before.
  - [ ] Custom-dim happy path: a new unit test at `tests/embedder/dim-parameterization.test.ts` creates `PristineLocal.create({ embedder: { engine: 'local', model: '<test-model>', dim: 1024 } })`, verifies the resulting `vec_windows` DDL contains `float[1024]` (via `PRAGMA table_info` or DDL inspection), and a `storeAsync` → `searcher.vectorSearch` round-trip succeeds.
  - [ ] Mismatch error path: configure `dim=1024` with an embedder that produces 768; assert `embed()` throws `InvalidArgumentError` with both dims named in the message.
...
- **Dependencies:** Stories 0 + 1. Story 0's parameterized `dim` lets the harness wrapper supply a per-candidate dim (critical for Qwen3 truncation: the wrapper can request `dim=768` while the engine produces 1024 natively, and the wrapper L2-renormalizes the slice).
- **Acceptance criteria:**
...
- **Acceptance criteria:**
  ...
  - **Conditional SDK default-config change deliverable (modularization invariant: shape unchanged from Story 0 baseline):**
    - [ ] **Decision rule (locked):** the SDK default-config change fires iff (a) the candidate is **in-process** (excludes Ollama candidates from eligibility because the SDK no-config default must not require a sidecar daemon), AND (b) the candidate's **dense-only** NDCG@10 paired bootstrap 95% CI lower bound is **strictly greater than** Nomic v1.5's dense-only NDCG@10 paired bootstrap 95% CI upper bound (operational test: `candidate.ndcg10.dense.ci_lower > nomic.ndcg10.dense.ci_upper`). Tie or overlap → no change. The only Story 4 candidate that meets eligibility (a) is `gte-modernbert-base`; the two Ollama candidates (`embeddinggemma:300m`, `qwen3-embedding:0.6b`) are recommendation-only, never SDK default. If the rule fires, change the **default `EmbedderConfig` value** in `src/client.ts` from `{ engine: 'local', model: 'nomic-embed-text-v1.5', dim: 768 }` to `{ engine: 'local', model: 'gte-modernbert-base', dim: 768 }`. If the rule does NOT fire, keep Nomic v1.5 as the SDK fallback; the docs still recommend the candidate but consumers must opt in via explicit config. Document the decision + which arm of the rule fired in the Story 5 PR body.
...
- **Functional verification:**
  ...
  - [ ] If the SDK default-config changed: **modularization-invariant audit** — let `STORY0_SHA` be the merge-commit sha of Story 0 into `sprint-017` (captured at Story 0 close and recorded in the Story 5 PR body). The check `git diff $STORY0_SHA..HEAD -- src/core/interfaces.ts src/embedder/index.ts src/index.ts` returns **zero changes** (proves Story 5's commits do not modify the public surface; Story 0 already absorbed the one-time additive `dim` change). The barrel must be byte-identical, not just export-list-identical, because a type-signature change in a re-exported symbol is also a surface change.
