...
- **Acceptance criteria:**
  ...
  - [ ] The harness ships RED outer-loop tests for Story 2 (`client.buildSessionVector` populates session leg) and Story 3 (`client.drainEmbedQueue` drains queue), each failing with TS error: `Property 'buildSessionVector' does not exist on type 'PristineLocal'` and `Property 'drainEmbedQueue' does not exist on type 'PristineLocal'` respectively (TypeScript-level failure, not a runtime "not implemented" throw — the methods don't exist yet at all).
  - [ ] Story 4's outer-loop assertion is a **runtime** check using `expect(barrel.IngestQueue).toBeUndefined()` (and the same for `IngestTask`, `IngestQueueConfig`, `Memory`) — `barrel` is `await import('../../src/index.js')`. Currently FAILS (those exports exist); Story 4's removal flips it GREEN. No `tsd` / `@ts-expect-error` mechanism.
...
- **Technical notes:**
  ...
  - **Export-absence check is runtime, not type-level.** Locked: the Story 4 RED assertion is `expect(barrel.IngestQueue).toBeUndefined()` etc., where `barrel = await import('../../src/index.js')`. Rationale: a runtime check stays green-by-default once Story 4 ships (the property is genuinely undefined on the imported namespace), whereas a `@ts-expect-error`-based mechanism would need a follow-up edit to remove the directive after Story 4 lands. No `tsd`, no `@ts-expect-error`.
