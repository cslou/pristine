...
- **Functional verification:**
  ...
  - [ ] Inner-loop unit test: deny-on-parse-uncertainty — `validateSqlAccess` on inputs that REACH the table-extraction phase but are STRUCTURALLY ambiguous to the static parser (NOT inputs that hit the non-SELECT keyword set first AND NOT inputs that look like a table identifier the parser then rejects via allowlist — those exercise different branches). Locked example inputs: `'SELECT * FROM (VALUES (1, 2)) AS t(a, b)'` (inline VALUES subquery — no identifier to extract); `'SELECT * FROM (SELECT 1 UNION SELECT 2) AS sub'` (inline derived-table subquery — extractor finds no top-level table reference); `'SELECT 1 FROM messages_public WHERE EXISTS (SELECT * FROM (VALUES (1)) AS x)'` (nested ambiguous subquery inside an otherwise-valid query). Each must throw `InvalidSqlError`. Pass condition: error class is `InvalidSqlError` AND error message contains the offending input substring or the literal phrase "parse uncertainty" (the parser surfaces one of the two — locked at story start). **Out of scope for this FV item (would exercise the wrong branch):** `generate_series(...)` and similar table-valued-function references look like a normal table identifier to a static parser and get caught by allowlist-mismatch, not parse-uncertainty.
...
- **Regression verification:**
  ...
  - [ ] **Path (a) only:** bidirectional JSDoc cross-references confirmed — pass condition: each of the six per-file per-name checks below prints `≥ 1` to stdout (READ THE PRINTED INTEGER — `grep -c` exits with code 1 on zero matches, so `... && echo PASS` would silently chain-break on a missing reference; verify the printed count is ≥1, not the exit code):
    ...
    Equivalent exit-code-safe form: `for f in src/client.ts src/memory/searcher/index.ts; do for s in searchConversations 'searcher\\.ftsSearch' 'searcher\\.sql'; do n=$(grep -c "$s" "$f"); echo "$f $s $n"; [ "$n" -ge 1 ] || echo FAIL; done; done` — pass condition: zero `FAIL` lines.
