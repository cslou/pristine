...
- **Functional verification:**
  ...
  - [ ] Recipe-shipped static check (**branch-independent** — runs in both `extended` and `not extended` branches since spec §5.2 receives the recipe in both) — `grep -nE "client\.searcher\.sql" docs/specs/implementation-spec-005.md` returns ≥ 1 hit under the §5.2 reference-implementations heading. Pass condition: hit count ≥ 1 AND the SQL fence under §5.2 contains the literal `searcher.sql` invocation. This proves the recipe deliverable shipped to its canonical artifact regardless of allowlist outcome (the recipe-equivalence test at FV-1 only runs in the `extended` branch).
- **Regression verification:**
  - [ ] `npm run typecheck`, `npm run lint`, `npm run test:unit`, `npm run test:integration` — all clean after removal (regression target: existing call paths into `messages_public` / `conversations_public` / `summaries_public` searches still work after the SDK method is gone).
  - [ ] Caller-migration audit — same `grep -rn "searchConversations" src/ tests/` command as AC-4, but the RV-side reading verifies *no production caller still imports the removed symbol* (i.e., callers in `scripts/`, test fixtures, and any internal usage were migrated to the recipe — not just that the SDK method definition is gone). Pass condition: zero hits in `src/` and `tests/`. AC-4 proves the deliverable; RV proves no consumer regressed.
  - [ ] `bash .checks/pre-merge.sh` — pass condition: lint + typecheck + unit suite all pass.
...
- **Regression verification:**
  ...
  - [ ] Recipe-shipped audit (cross-story, branch-aware) — `grep -nE "client\.searcher\.sql" docs/specs/implementation-spec-005.md` returns ≥ 1 hit under the §5.2 reference-implementations heading regardless of Story 2's allowlist branch. Additionally, IF Story 2 documented `extended`, `grep -nE "snippet\(messages_fts" src/memory/searcher/index.ts` returns ≥ 1 hit (recipe also lives in the `searcher.sql` JSDoc). IF Story 2 documented `not extended`, the JSDoc grep is skipped (the recipe defers via spec §5.2 only). Pass condition: spec hit ≥ 1 unconditionally; JSDoc hit ≥ 1 if `extended`. This catches a "recipe missing from artifact" regression that per-story FV does not bound.
