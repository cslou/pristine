...
- **Technical notes:**
  ...
  - **Outer-loop test plan reference (illustrative, NOT prescriptive verbatim text):** the seven `it(...)` titles should encode the AC anchor (e.g. `@AC-Story4-1`, `@AC-Story3-1`, `@AC-Story2-1`, `@AC-Story5-1`) so failures map back to the sourcing story.
...
- **Functional verification:**
  ...
  - [ ] Inner-loop unit test: deny-on-parse-uncertainty — `validateSqlAccess` on inputs that REACH the table-extraction phase but are ambiguous there (NOT inputs that hit the non-SELECT keyword set first — those exercise a different branch). Locked example inputs: `'SELECT * FROM (VALUES (1, 2)) AS t(a, b)'` (inline VALUES subquery — table-extractor can't resolve), `'SELECT * FROM generate_series(1, 10)'` (table-valued function — not a real table reference), `'SELECT * FROM (SELECT 1 UNION SELECT 2)'` (inline derived table). Each must throw `InvalidSqlError`. Pass condition: error class is `InvalidSqlError` AND error message contains the offending input substring or the literal phrase "parse uncertainty" (the parser surfaces one of the two — locked at story start).
...
- **Functional verification:**
  ...
  - [ ] DoS test wall-time check: `time npx vitest run tests/integration/searcher-sql.test.ts -t '\\(d\\) DoS'` (filter matches the locked describe title `'(d) DoS / row-cap escape'`; parentheses escaped because `-t` is a regex). Pass condition: each DoS test completes within ≤12s wall time AND vitest's reporter shows `5 passed` (or higher) for the (d) sub-class — NOT `0 tests` (which would indicate the filter matched nothing).
...
- **Regression verification:**
  ...
  - [ ] **Path (a) only:** bidirectional JSDoc cross-references confirmed — pass condition: each of the six per-file per-name `grep -c` checks below returns ≥1 (machine-verifiable; flat multi-file `grep -nE` is NOT acceptable because hits in one file can mask gaps in the other):
...
- **Technical notes:**
  ...
  - **If decision is (c) — recipe shape.** The recipe must be a copy-pasteable raw-SQL string in the JSDoc and spec, NOT a prose description. Example shape (validate at story start; assumes Story 3 extended allowlist to include `messages_fts`):
    ...
    //      validate / clamp before calling. Edge cases the AC range guards against: `limit = 0` returns zero
    //      rows from SQLite (valid SQL, surprising UX); `limit = -1` causes SQLite to interpret `LIMIT -1` as
    //      "unlimited" so the row-cap cursor becomes the only bound (still bounded, but degraded UX).
