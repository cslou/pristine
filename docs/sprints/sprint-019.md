...
- **Implementation spec:** `docs/specs/implementation-spec-005.md` — §5.1.3 lists `searcher.sql(sql, opts?): Row[]` as a primitive (post-rebase: raw SQL only, no DSL); §15 Flow 3 describes the validation pipeline (parse + validate access surface → read-only conn → progress handler → row-cap cursor → execute); §16 Phase 5 enumerates stories P5-S1 through P5-S4; §8.6 specifies the adversarial test suite the privacy boundary must survive. Spec touchups in Story 5 align §5.1.3 and §15 to the rebased surface shape.
...
- **Acceptance criteria:**
  ...
  - [ ] The default allowlist is `new Set(['messages_public', 'conversations_public', 'summaries_public'])` — exposed as `DEFAULT_PUBLIC_VIEW_ALLOWLIST` from the module for re-use in tests / future spec evolution. **Story-start decision (locked verbatim):** verify whether `messages_fts` exposes only the indexed `content` column (FTS5 shadow tables typically do); if so, AND if Story 6 path-(c) confirms FTS MATCH access is needed for the `searchConversations` recipe, extend the default allowlist to include `messages_fts`. If FTS shadow exposes any internal column (`metadata`, `parent_message_id`), DO NOT extend — bounce the FTS-recipe requirement to a follow-up sprint that ships a `messages_fts_public` view. Document the verification + decision in the Story 3 PR body.
...
- **Acceptance criteria:**
  ...
  - [ ] `searcher.sql` types are exported from `src/index.ts`: `SqlOpts` (defined in this story's AC-3), `Row` (defined in Story 2 AC-6), `InvalidSqlError` (Story 3 AC-2), `QueryTimeoutError` (Story 2 AC-3). No DSL types — the primitive accepts raw SQL only.
...
- **Acceptance criteria:**
  - [ ] Adversarial cases **appended to `tests/integration/searcher-sql.test.ts`** (locked — single file keeps the harness's adversarial coverage co-located with its happy-path coverage and avoids fragmenting test-discovery globs) — at least 20 adversarial test cases organized into 4 attack classes under a top-level `describe('searcher.sql adversarial suite', ...)` block:
    ...
    - **(b) Internal-table SELECT** (≥ 5 cases): `SELECT * FROM messages`, `SELECT * FROM conversations`, `SELECT * FROM vec_windows`, `SELECT * FROM vec_sessions`, `SELECT * FROM messages_fts`. Each must throw `InvalidSqlError`. **NOTE:** if Story 3's story-start verification extends the default allowlist to include `messages_fts` (because Story 6 path-(c) needs FTS MATCH for the `searchConversations` recipe AND the FTS shadow exposes only `content`), drop `messages_fts` from this case set and add a substitute internal-only table (e.g., `embed_jobs`, `migrations` — pick whatever exists in `src/conversations/store.ts` outside the public-view set).
  ...
  - [ ] `docs/specs/implementation-spec-005.md` §15 Flow 3 + §5.1.3 + §16 Phase-5 + Flow 5 updated with: (a) `client.searcher.sql(sql, opts?)` as the single consumer surface, raw SQL only — no DSL/translator step in the diagram; (b) **DSL example block at §15 Flow 3 (around line 944) DELETED** (DSL was dropped at sprint rebase); (c) **Flow 5 §15 (`query_memory` reference impl, around line 996) updated** to show raw-SQL composition rather than DSL — the LLM emits SQL, the handler validates + executes; (d) §5.1.3 wording "scoped DSL (preferred) or raw SQL (escape hatch)" replaced with "raw SQL with positional `?` parameter binding"; (e) §16 P5-S2 line about `src/memory/searcher/sql.ts` confirmed — single SQL module, no separate DSL translator. Cross-reference: spec §5.1.3 and §15 must agree on the surface shape.
...
- **Acceptance criteria:**
  ...
  - [ ] Decision is one of: **(a) Keep with doc clarification** — JSDoc on `searchConversations`, `searcher.ftsSearch`, AND `searcher.sql` explaining when to reach for each; **(b) Align with project-scoping** — make `searchConversations` accept `projectId` (additive — `userId` becomes optional) and harmonize the contract with `searcher.*`; **(c) Remove + recipe** — delete the method, ship the concrete `searcher.sql` raw-SQL recipe in JSDoc + spec §5.2 reference-implementations.
...
- **Technical notes:**
  ...
  - **If decision is (c) — recipe shape.** The recipe must be a copy-pasteable raw-SQL string in the JSDoc and spec, NOT a prose description. Example shape (validate at story start; assumes Story 3 allowlist extension to include `messages_fts`):
    ```ts
    ...
    ```
    **Story-start verification:** confirm Story 3 extended the allowlist to include `messages_fts`. If NOT (because the FTS shadow leaks internal columns), the recipe is unreachable in this sprint — fall back to (a) Keep with a doc note that the recipe lands once `messages_fts_public` view ships in a follow-up sprint.
- **Priority:** Must-have
