...
- **Functional verification:**
  ...
  - [x] Run the audit command against reachable git history and confirm it exits 0 or produces only documented false positives. Evidence: final integration rerun scanned 932 reachable commits before the report commit and 92 unique history findings, all false positives; recursive false-positive rows from generated audit reports are filtered from history results.
