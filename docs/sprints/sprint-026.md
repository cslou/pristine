...
- **Acceptance criteria:**
  ...
  - [x] If old type names remain exported, `src/index.ts` and the public type fixture make clear they are compatibility exports, not the canonical naming users should copy first. Evidence: legacy aliases in `src/client.ts` have `@deprecated`; public type fixture imports both new and legacy types to prove compatibility.
