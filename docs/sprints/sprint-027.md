...
- **Acceptance criteria:**
  - [x] `README.md` is rewritten as a public landing page with: product summary, local-first/privacy promise, install command, minimal quickstart, core API overview, Pi-dev integration pointer, docs links, status, license, and security/reporting links. **Pass condition:** all listed sections are present, README stays under 220 lines, and deeper API/how-to detail links to Vocs instead of being duplicated inline. Evidence: README is 90 lines; `npm run verify:docs` passed.
  - [x] README links point only to existing public files/routes such as `docs/pages/index.mdx`, `docs/pages/quickstart.mdx`, `docs/pages/api.mdx`, `docs/pages/privacy.mdx`, `docs/pages/pi-dev.mdx`, `examples/pi-dev/README.md`, `SECURITY.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, and `LICENSE`. **Pass condition:** `npm run verify:docs` reports no missing README links. Evidence: `npm run verify:docs` passed.
  - [x] README contains no links to removed internal paths and no internal planning terms such as sprint docs, implementation specs, story checklists, or security-audit files. **Pass condition:** `rg "docs/sprints|docs/specs|docs/stories|docs/security-audits|implementation-spec|Sprint [0-9]|Story Checklist" README.md` returns no hits. Evidence: command returned no hits.
  - [x] README uses the current public memory and Pi tool names: `store`, `recall`, `forget`, and `pristine_recall`. **Pass condition:** README contains no deprecated source-chunk or removed Pi tool references. Evidence: stale-name audit returned no hits.
- **Functional verification:**
  - [x] Run `npm run verify:docs`. **Pass condition:** README and public docs links/snippets validate successfully. Evidence: passed.
  - [x] Run `rg "docs/sprints|docs/specs|docs/stories|docs/security-audits|implementation-spec|pristine_vector_search|indexSourceChunks|searchSourceChunks|deleteSourceChunks" README.md docs/pages/*.mdx`. **Pass condition:** no unapproved stale/internal hits remain in public-facing docs. Evidence: command returned no hits.
- **Regression verification:**
  - [x] Run `npm run verify:package`. **Pass condition:** package contents remain valid and do not accidentally include removed internal docs beyond the intended npm package files. Evidence: passed.
  - [x] Run `npm run test:smoke`. **Pass condition:** public package entrypoint and public API smoke tests still pass after README/docs changes. Evidence: passed.
- **Manual-only verification:** N/A — no manual-only verification required.
- **Planned commits:**
