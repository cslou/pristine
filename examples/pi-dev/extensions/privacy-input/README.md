...
On macOS, `Pristine.create()` now defaults to store-namespaced Keychain-backed RSA keys for privacy vault access. Host wrappers that want this default should avoid passing `keysDir`; supplying `keysDir` forces the filesystem key path instead. First local `redact`, `reveal`, or `resolveSensitive` use for a user/store may create a Keychain keypair and may trigger a one-time system prompt depending on local Keychain policy.
