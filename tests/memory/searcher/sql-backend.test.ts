    ...
    async () => {
      ...
      await expect(
        backend.executeReadOnly(
          ...
          [],
          { rowCap: MAX_ROW_CAP, timeoutMs: 200 },
        ),
      ).rejects.toBeInstanceOf(QueryTimeoutError);
      const elapsed = Date.now() - start;
      // Generous CI slack: prove we aborted well before the cartesian's
      // natural completion (many seconds at 50KB/row over 4M rows).
...
describe('createSqlBackend.withTimeout', () => {
  it(
    'is exposed on the SqlBackend and yields rows when the query is fast, throws QueryTimeoutError when slow',
    { timeout: 7000 },
    () => {
      const backend = createSqlBackend({ dbPath });
      ...
      try {
        ...
        const fastRows = Array.from(backend.withTimeout(fastStmt, [], 1000));
      ...
      try {
        const slowStmt = slowDb.prepare<unknown[], { n: number }>(
          'SELECT a.n FROM nums a, nums b, nums c',
        );
        const consume = (): void => {
          // Drain the generator; the per-iteration check throws before
          // the cartesian completes its 8B-row search space.
          Array.from(backend.withTimeout(slowStmt, [], 200));
        };
